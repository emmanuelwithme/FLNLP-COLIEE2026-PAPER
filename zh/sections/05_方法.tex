...
本研究的方法並非從零開始，而是沿著「先重現，再修正，再整合」的流程逐步形成。本節依最終系統說明：基線模型、長文本 encoder、對比式 supervised fine-tuning、動態負樣本選取、法律領域繼續預訓練、scope filtering、learning-to-rank，以及 post-processing。為簡潔起見，本文將美國判決書上的 continued pretraining 稱為 DAP（domain-adaptive pretraining），將 COLIEE Task~1 上的對比式有監督微調稱為 SFT（supervised fine-tuning）。
...
\begin{figure*}[t]
  ...
  \Description{Task 1 系統流程圖，分成訓練驗證前處理、模型訓練與測試輸出三個區塊。流程包含原始訓練案例清理與切分、BM25 與年份範圍建立、BM25 負樣本池與動態負樣本驅動的對比式 SFT 和 LTR 訓練，以及測試資料經 lexical 打分、dense 打分、LTR 重排與最後後處理後輸出答案。}
  \label{fig:task1-system-pipeline}
\end{figure*}

\subsection{BM25 與 SAILER 基線}

我們保留 BM25 與 SAILER 作為主要基線。BM25 代表 lexical retrieval 的強基線；SAILER 則對應 THUIR@COLIEE 2023 使用的 legal encoder \cite{li2023thuir}。在本文中，SAILER 不再重新 fine-tune，而是直接使用既有模型進行嵌入與相似度計算\footnote{\url{https://huggingface.co/CSHaitao/SAILER_en_finetune}}。這個設定其實對 SAILER 偏有利，因為其公開 fine-tuning 模型很可能已經看過與本文 valid split 重疊、或至少高度相近的資料分布。
...
對於每篇判決書，我們使用 encoder 開頭的 `[CLS]` 向量表示作為 dense retrieval 嵌入，並在實作中再經 projector 與 L2 normalization；排名則同時比較 dot product 與 cosine similarity。
...
除了 task-specific SFT 之外，我們也先在美國判決書語料上對 ModernBERT 進行 DAP。具體而言，DAP 使用 Common Pile CaseLaw Access Project subset，並以 masked language modeling 訓練 backbone encoder。目的是讓 encoder 先適應法律文本中的長句、引文格式與程序語言。受限於目前 GPU 條件，continued pretraining 約訓練 60 小時，仍只覆蓋 0.384 epoch，因此本文將它視為「仍未訓練飽和」但有幫助的法律領域適應步驟。
